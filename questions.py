...
# Data Master Topic
target_topics = ['bot_discord', 'computer_vision', 'flask', 'nlp']
topic_list = {}

# 1. Ambil semua topik yang ada di DB yang namanya ada di dalam list target
existing_topics = Topic.query.filter(Topic.name.in_(target_topics)).all()

# 2. Ubah hasil query menjadi dictionary agar mudah dicari di memori
# Contoh hasil: {'bot_discord': <Topic Object>, 'flask': <Topic Object>}
existing_topics_dict = {topic.name: topic for topic in existing_topics}

# 3. Looping untuk mengecek dan mengisi topic_list
for name in target_topics:
    if name in existing_topics_dict:
        # Jika sudah ada di DB, gunakan yang sudah ada
        topic_list[name] = existing_topics_dict[name]
    else:
        # Jika belum ada, buat baru dan masukkan ke session
        baru = Topic(name=name)
        db.session.add(baru)
        topic_list[name] = baru

# 4. Commit sekali saja di akhir
db.session.commit()
...
bot_discord = [
        Question(text="Platform tempat kita harus mendaftarkan bot baru dan mendapatkan token sebelum mulai menulis kode disebut...",
                topic_id = topic_list['bot_discord'].id,
...
ai_vision = [
        Question(text="Dalam pemrosesan gambar, proses mengubah gambar berwarna menjadi hitam-putih-abu-abu disebut dengan...",
                topic_id=topic_list['computer_vision'].id,
                option_a="Resizing", 
                option_b="Grayscale", 
...
db.session.commit()
print("Data kuis bot_discord, computer_vision, flask, dan NLP berhasil dimasukkan!")
...
def seed_questions():
    try:
        ...
        db.session.rollback()
        print("Akun Admin sudah ada atau sedang dibuat, proses seeding dilewati secara aman.")